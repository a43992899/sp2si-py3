...
def random_pred(model_list=['PMTL'], n_samp=2, min_length=1.0, fld=test_fld, psongs=test_psongs):
    ...
    for data in dataload:
        ...
        for idx in range(len(model_list)):
            ...
            # Save predictions
            sf.write(saving_dir + 'original_speech_' + str(samp_idx) + '.wav', time_inp_orig, sr)
            sf.write(saving_dir + 'stretched_speech_' + str(samp_idx) + '.wav', time_inp_phase, sr)
            sf.write(saving_dir + 'true_singing_' + str(samp_idx) + '.wav', time_target_phase, sr)
            sf.write(saving_dir + 'predicted_singing_' + str(samp_idx) + cur_model + '.wav', time_pred, sr)
...
def eval_sys(model_list=['PMTL', 'PMSE', 'B1', 'B2'], n_samp=30, min_length=1.0, random=True, fld=test_fld, psongs=test_psongs):
    ...
    for data in dataload:
        ...
        for idx in range(len(model_list)):
            ...
            sf.write(true_file, time_target_phase, sr)
            sf.write(pred_file, time_pred, sr)
...
def eval(net1, net2, speech_file_loc, melody_file_loc):
    ...
    idx2 = np.array(list(range(n_frames)))
