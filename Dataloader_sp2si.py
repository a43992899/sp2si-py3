...
class nus_samp(Sampler):
    ...
    def RandomSelect(self):
        if (self.use_word == False or self.use_word == True):
            idx_arr = list(range(len(self.all_samp_details)))
            #idx_arr = [1761, 74]
            if (self.randomize):
                ...
                idx_arr = list(range(start_idx, end_idx))
                print(idx_arr)
...
def pitch_pyin(start_samp_num, usr, snum, out_shape, hop_size=128, fs=16000, nfft=1024):
    ...
    idx2 = np.array(list(range(pc.shape[1])))
    pc[idx1, idx2] = 1
    pc[-1] = 1 * pc[0]
    pc[0] = 0 * pc[0]
    return pc
...
def remove_silent_frames(x, dyn_range=40, framelen=1024, hop=128):
    ...
    for i in range(x_frames.shape[0]):
        x_sil[list(range(i * hop, i * hop + framelen))] += x_frames[i, :]
    return x_sil
