...
#        print('time at: ', t_idx)
        for mm in range(0, len(LOCS)):
            if LOCS[mm] >= half_ps and LOCS[mm] < M - half_ps and counter<300000:# and PKS[mm]> 0.5*max(Z[:,t_idx]):
                patch = Z[np.ix_(list(range(LOCS[mm]-half_ps, LOCS[mm]+half_ps+1)), list(range(t_idx-half_ps, t_idx+half_ps+1)))]
...
#            print(fi)
    ...
    PredContour = PredContour[list(range(half_ps, N-half_ps))]
    
    for k in range(len(PredContour)):
        if PredContour[k]>1:
            PredContour[k] = CenFreq[PredContour[k].astype('int')]
    
    Z = Z[:, list(range(half_ps, N-half_ps))]
#    print(t.shape)
...
#    plt.plot(result[:,0],result[:,1])
    ...
    melody_extraction(args.InFile, args.OutFile)
